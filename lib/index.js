...
module.exports = async (flags, projectName) => {
  ...
  const logDebug = text =>
    flags.debug && console.log(chalk.dim(`> ${chalk.green('DEBUG')} - ${text}`))
  ...
  if (path.basename(dir) === 'pages') {
    throw new Error(
      'Your root directory is named "pages". This looks suspicious. You probably want to go one directory up.'
    )
  }
  ...
  try {
    ...
    if (flags.init) {
      ...
      if (unsafeInit && !flags.force) {
        throw new Error(
          `${chalk.bold(name)} already has a ${chalk.bold('package.json')}, which would be overwritten`
        )
      }
    }
    ...
    const shouldChangeDir = flags.init && present && cwdIsDir
      ? ''
      : `cd ${relativePath} && `
    ...
    if (!process.env.CI) {
      // Skip copy when running travis/CI
