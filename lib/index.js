...
async function nn(argv) {
  ...
  const logDebug = text => argv.debug && console.log(chalk.dim(`> ${chalk.green('DEBUG')} - ${text}`))
  ...
  if (argv.help) {
    ...
    ${chalk.dim('Add optional dependencies')}

      ${availableExtensions.map(ext => `\t\b\b--${ext}`).join('\n')}
    ...
    ${chalk.dim('Examples')}
      ...
      Create 'my-new-blog', and add Glamor as a dependency
        ...
        ${chalk.cyan('$ nn my-new-blog --glamor')}
    `)
    process.exit(0)
  }
